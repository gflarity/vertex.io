...
(function($) {
  ...
  $.extend($.futon, {
    ...
    CouchIndexPage: function() {
      ...
      var urlParts = location.pathname.substr(1).split("/");
      
      // api/v1/
      urlParts.shift();
      urlParts.shift();
      var devName = decodeURIComponent(urlParts.shift());
            ...
            $.couch.db(data.name).create({
              ...
              success: function(resp) {
                location.href = "database.html?api/v1/" + encodeURIComponent(devName) + 
                  "/db/" + encodeURIComponent(data.name);
                callback();
              }
            });
          }
          ...
          success: function(dbs) {
            ...
            $.each(dbsOnPage, function(idx, dbName) {
              $("#databases tbody.content").append("<tr>" +
                "<th><a href='database.html?api/v1/" + encodeURIComponent(devName) + 
    ...
    CouchDatabasePage: function() {
      ...
      this.newDocument = function() {
        location.href = "document.html?api/v1/" + encodeURIComponent(devName) + 
      ...
      this.updateDesignDocLink = function() {
        if (viewName && /^_design/.test(viewName)) {
          ...
          $("#designdoc-link").attr("href", "document.html?api/v1/" +
            encodeURIComponent(devName) + "/db/" + 
      ...
      this.updateDocumentListing = function(options) {
        ...
        options.success = function(resp) {
          ...
          for (var i = 0; i < Math.min(perPage, resp.rows.length); i++) {
            ...
            if (row.id) {
              key = key.replace(/\\"/, '"');
              var rowlink = 'api/v1/' + encodeURIComponent(devName) + 
                "/db/" + encodeURIComponent(db.name) +
        ...
        function handleResult(doc, revs) {
          ...
          if (revs.length > 1) {
            ...
            if (currentIndex < revs.length - 1) {
              ...
              $("#paging a.prev").attr("href", "?api/v1/" + encodeURIComponent(devName) +
                "/db/" + encodeURIComponent(dbName) +
                "/" + $.couch.encodeDocId(docId) + "@" + prevRev);
            }
            if (currentIndex > 0) {
              var nextRev = revs[currentIndex - 1].rev;
              $("#paging a.next").attr("href", "?api/v1/" + encodeURIComponent(devName) +
                "/db/" + encodeURIComponent(dbName) +
