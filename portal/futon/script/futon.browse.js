  ...
  $.extend($.futon, {
          ...
          submit: function(data, callback) {
            ...
            $.couch.db(data.name).create({
              ...
              success: function(resp) {
                location.href = "database.html?" + encodeURIComponent(devName) + 
          ...
          success: function(dbs) {
            ...
            $.each(dbsOnPage, function(idx, dbName) {
              $("#databases tbody.content").append("<tr>" +
                "<th><a href='database.html?" + encodeURIComponent(devName) + 
    ...
    CouchDatabasePage: function() {
      ...
      this.newDocument = function() {
        location.href = "document.html?" + encodeURIComponent(devName) + 
      ...
      this.updateDocumentListing = function(options) {
        ...
        options.success = function(resp) {
          ...
          for (var i = 0; i < Math.min(perPage, resp.rows.length); i++) {
            ...
            if (row.id) {
              key = key.replace(/\\"/, '"');
              var rowlink = encodeURIComponent(devName) + 
                "/" + encodeURIComponent(db.name) +
        ...
        function handleResult(doc, revs) {
          ...
          if (revs.length > 1) {
            ...
            if (currentIndex < revs.length - 1) {
              ...
              $("#paging a.prev").attr("href", "?" + encodeURIComponent(devName) +
                "/" + encodeURIComponent(dbName) +
                "/" + $.couch.encodeDocId(docId) + "@" + prevRev);
            }
            if (currentIndex > 0) {
              var nextRev = revs[currentIndex - 1].rev;
              $("#paging a.next").attr("href", "?" + encodeURIComponent(devName) +
                "/" + encodeURIComponent(dbName) +
              ...
              if (docRev != null) {
                db.openDoc(docId, {rev: docRev,
                  error: function(status, error, reason) {
                    ...
                    location.href = "?" + encodeURIComponent(devName) +
                      "/" +encodeURIComponent(dbName) +
                      "/" + $.couch.encodeDocId(docId);
                  },
                  success: function(doc) {
